...
- `custom_components/ai_expose_entities/` - Main integration code
...
- Integration logs: `custom_components.ai_expose_entities: debug` in `config/configuration.yaml`
...
- **Domain:** `ai_expose_entities`
- **Title:** AI Expose Entities
- **Class prefix:** `AIExposeEntities`

**When creating new files:**

- Use the domain `ai_expose_entities` for all DOMAIN references
- Prefix all integration-specific classes with `AIExposeEntities`
- Use "AI Expose Entities" as the display title
...
- Inherit from platform base + `AIExposeEntitiesEntity`
...
- `tests/` mirrors `custom_components/ai_expose_entities/` structure
