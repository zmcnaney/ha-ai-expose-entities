...
This document describes all configuration options and settings available in the AI Expose Entities custom integration.
...
### `ai_expose_entities.example_service`
...
```yaml
service: ai_expose_entities.example_service
target:
  ...
  - alias: "Call service at sunset"
    trigger:
      - trigger: sun
        event: sunset
    action:
      - action: ai_expose_entities.example_service
        target:
          entity_id: switch.device_name_switch
        data:
          parameter: "mode"
  ...
  input:
    sensor_entity:
      ...
      selector:
        entity:
          domain: sensor
          integration: ai_expose_entities
    threshold:
      name: Threshold
      selector:
        number:
...
- [GitHub Issues](https://github.com/zmcnaney/ha-ai-expose-entities/issues) - Report problems
